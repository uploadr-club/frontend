...
function loadData() {
  getUserData().then((data) => {
    ...
    $("#sxcu_button")[0].onclick = () => {
      window.open(
        `https://api.uploadr.cloud/api/v1/download/sxcu?api_key=${data.user.api_token}&embed=no`
      );
    };
  });
}
...
async function saveDiscordSettingsFetch(
  ...
  // noinspection SpellCheckingInspection
  let req = await fetch(
    "https://api.uploadr.cloud/api/v1/user/discord_settings",
    {
      method: "POST",
      headers: headers,
      body: JSON.stringify({
...
async function changePasswordFetch(password) {
  ...
  let req = await fetch(
    "https://api.uploadr.cloud/api/v1/user/change_password",
    {
      method: "POST",
      headers: headers,
      body: JSON.stringify({
