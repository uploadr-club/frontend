  ...
  if (dashboard_urls.indexOf(window.location.pathname) > -1) {
    ...
    getUserData().then((data) => {
      ...
      if (!jQuery.isEmptyObject(data.discord)) {
        // noinspection JSUnresolvedVariable
        $("#usernameDropDownSpan")[0].innerHTML =
          `<img style="border-radius: 50%;" alt="Profile Picture" width="45" src="https://cdn.discordapp.com/avatars/${data.discord.discord_data.user.id}/${data.discord.discord_data.user.avatar}.png?size=256">  ${data.user.username}`;
      } else {
